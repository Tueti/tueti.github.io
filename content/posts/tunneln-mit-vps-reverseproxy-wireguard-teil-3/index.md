...
Dinge, wie `nextcloud.meinedomain.de` oder eventuell auch `photos.meinedomain.de` möchten wir möglicherweise öffentlich erreichbar machen. Und da kommen wir zum zweiten Thema. Nachdem wir den ersten Teil erledigt haben, möchte ich den Dienst _Authentik_ als Docker Container hochziehen und mit diesem Dienst eine sichere Authentifizierung der Nutzer erzwingen. Dann könnten diese URLs zwar aus dem Internet erreicht werden, aber anstelle der direkten Loginseite des jeweiligen Dienstes (welcher mehr oder weniger sicher sein kann), öffnet sich die Authentifizierung für Authentik. Hier können wir Nutzer zwingen, eine Zwei-Faktor-Authentifizierung durchzuführen, können die erlaubten Benutzer filtern und so eine sichere(re) Anmeldung ermöglichen. Erst nach der Authentifizierung bei Authentik würden die User dann zu unserem dahinterliegenden Service kommen. Und das Beste ist, viele Dienste ermöglichen ein Single-Sign-On mit Apps, wie Authentik. Also nach erfolgreicher Authentifizierung bei Authentik wird man _automatisch_ beim dahinterliegenden Dienst eingeloggt. Man braucht sich dann nicht doppelt anzumelden.
